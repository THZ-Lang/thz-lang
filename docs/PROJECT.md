...
5. **Autonomia AOT & Runtime Rust:** Compilador self-hosted em THZ (`compilador/*.thz`), backend LLVM IR/Clang, runtime nativo oficial em Rust (`src/runtime_rs/`) com C ABI pura e geração de binários nativos autônomos sem dependência de JVM.
...
| [`RUNTIME_NATIVO.md`](RUNTIME_NATIVO.md) | Runtime Rust C ABI, Arenas $O(1)$, SIMD AVX2/512, Crypto, WASM | Runtime |
| [`BRASIL_DIGITAL_E_SNAPSHOT_ENGINE.md`](BRASIL_DIGITAL_E_SNAPSHOT_ENGINE.md) | PIX, Boletos, CEPs `.thzdbi`, feriados e snapshots compactados | Brasil / Cache |
| [`ENGENHARIA_DE_DADOS_E_ANALYTICS.md`](ENGENHARIA_DE_DADOS_E_ANALYTICS.md) | Suíte DAX, funções estatísticas, planilhas e limpeza ETL | Analytics |
| [`CONECTORES_BANCO_E_MENSAGERIA.md`](CONECTORES_BANCO_E_MENSAGERIA.md) | JPA/ORM, Raw SQL, Busca Vetorial KNN, RabbitMQ, Kafka, SQS | Conectores |
...
| [`TESTES_E_BENCHMARKS.md`](TESTES_E_BENCHMARKS.md) | JUnit 5 testes, goldens, paridade, JMH, `write-tests` | Qualidade |
| [`LSP_VSCODE.md`](LSP_VSCODE.md) / [`API_REST.md`](API_REST.md) | LSP4J + DAP + VS Code + Spring Boot 11 endpoints | Tooling |
...
| [`ADRs/`](ADRs/README.md) | 6 ADRs (LLVM, Arena, i128, FlatLaf, Sintaxe Moderna...) | Decisões |
| [`TROUBLESHOOTING.md`](TROUBLESHOOTING.md) | FAQ por área (build, SIMD, arena, LSP, GraalVM, LLVM, Docker) | Suporte |
| [`CLI_E_TOOLING.md`](CLI_E_TOOLING.md) | `thz check/run/dev/fmt/doc/audit/ui/ir/gui/agent/release` + IDE/LSP | CLI |
...
| [`RELATORIO-EVOLUCAO.md`](RELATORIO-EVOLUCAO.md) + [`CHANGELOG.md`](../CHANGELOG.md) | Linha do tempo + changelog SemVer 2.0.0 | Histórico |
...
- [x] **Fase Multi-Módulo JVM 25:** Separação estrita em `thz-core`, `thz-cli`, `thz-gui`, `thz-lsp`, `thz-agent`, `thz-bench` e `thz-api`.
...
- [x] **Fase Self-Hosting & Autonomia Total:** Compilador self-hosted em THZ (`compilador/*.thz`) + Pipeline AOT LLVM Clang Dual-OS (`scripts/build-llvm.ps1`) com runtime nativo Rust (`src/runtime_rs/`), gerando binários executáveis independentes.
- [x] **Fase Paradigma Dual Moderno (v0.4.0):** Sintaxe ergonômica unificada ("Kotlin/Rust Corporativo"), blocos `{ ... }`, `fn`, `struct`, aliases de tipos, protocolo `thz release` e agente autônomo `thz agent`.
- [ ] **Fase Próxima:** Fatias Zero-Copy com Apache Arrow IPC e otimizações vetoriais AVX-512.
