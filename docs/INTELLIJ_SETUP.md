...
| JDK 25 | GraalVM JDK 25 (Recomendado) ou OpenJDK 25 | `scoop install graalvm25-jdk` (Win) |
| Gradle | 8.x (wrapper incluso) | `./gradlew` — não instale global |
| Clang + MinGW | LLVM + MinGW GCC | `scoop install llvm mingw` (para `build-llvm.ps1`) |
...
`gradle.properties:36` aponta `org.gradle.java.installations.paths=C:/Users/lucas/scoop/apps/graalvm25-jdk/current` — ajuste para seu `JAVA_HOME` se diferente.
...
1. `File → Project Structure → Project → SDK → Add → JDK → C:\Users\lucas\scoop\apps\graalvm25-jdk\current` (ou seu `JAVA_HOME`).
