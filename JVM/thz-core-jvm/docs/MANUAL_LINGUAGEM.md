...
4. **Gerenciamento por Bloco de Memória Temporária:** Processamentos em lote e escopos temporários utilizam alocação em bloco contíguo (`BlocoMemoria`), liberando toda a memória temporária de forma limpa e automática ao final da execução, sem sobrecarga de Garbage Collection.
...
## 8. Gerenciamento por Bloco de Memória Temporária (`USAR_BLOCO_MEMORIA`)

Permite alocar e descartar recursos temporários rapidamente sem sobrecarregar a memória do sistema:

```thz
USAR_BLOCO_MEMORIA(tamanhoBytes: 65536)
    # Toda a memória criada aqui é liberada automaticamente ao sair do bloco
    VARIAVEL temp : FATIA[INTEIRO32] <- [1, 2, 3, 4, 5]
    EXIBA "Processado em memória temporária isolada"
FIM_BLOCO_MEMORIA
```
