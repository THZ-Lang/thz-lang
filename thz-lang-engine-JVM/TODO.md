...
* [x] Fase 7 — Paridade JUnit (mvn test **58/58**)
...
* [x] Compilação nativa AOT GraalVM Native Image (`pom.xml` perfil `<profile id="native">` gerando `target/thz.exe`)
...
* [x] Idempotência Inteligente de Larga Escala (Wide-Language Application): cláusulas `IDEMPOTENTE` e `CHAVE_IDEMPOTENCIA` integradas em todos os níveis (Léxico, Sintático, AST, Semântico, Runtime `RegistroIdempotencia`, Interpretador O(1), Governança G4, IR G5, Formato canônico e DocGen)
