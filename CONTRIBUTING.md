...
- `JVM/`: Motor canônico em Java 25 (Gradle Multi-Módulo), composto por:
  - `thz-core-jvm/`: Núcleo da linguagem (Lexer, Parser, AST, Semântico, Runtime, DecimalFixo, IR, DocGen, Governança, DAP).
  - `thz-cli-jvm/`: CLI unificada com 17 comandos, REPL interativo e Dev Server.
  - `thz-gui-jvm/`: Desktop IDE Swing + FlatLaf (Editor, Gutter, Formulários Dinâmicos).
  - `thz-lsp-jvm/`: Servidor Language Server Protocol (LSP4J).
  - `thz-agent-jvm/`: Agente Autônomo de IA (Loop de raciocínio, RAG, backends LLM locais e API).
...
- `src/runtime_rs/`: Runtime nativo oficial em Rust (Dual-OS C ABI, Arenas $O(1)$, SIMD AVX2/512, Criptografia e WASM).
- `scripts/`: Automações de compilação AOT (`build-llvm.ps1`/`.sh`), testes (`test-all.ps1`/`.sh`) e empacotamento.
- `Extensions/thz-lsp-vscode/`: Extensão oficial para o VS Code e Antigravity IDE.
...
5. **Branch para Self-Hosting e Autonomia LLVM:**
   - Trabalhos no compilador self-hosted (`compilador/`), runtime Rust (`src/runtime_rs/`) ou codegen LLVM devem ser desenvolvidos na branch `feat/self-hosting-llvm-autonomy`.
