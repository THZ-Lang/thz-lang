...
DRIVER_THZ="$RAIZ/../thz-compiler/driver.thz"
if [ ! -f "$DRIVER_THZ" ]; then
    DRIVER_THZ="$RAIZ/compilador/driver.thz"
fi
bash "$RAIZ/scripts/build-llvm.sh" "$DRIVER_THZ" --saida "$DIST_BIN/thzc.elf"
