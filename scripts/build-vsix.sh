...
if [ -d "$RAIZ/../thz-vscode" ]; then
    PASTA_EXTENSAO="$RAIZ/../thz-vscode"
else
...
LSP_JAR_ORIGEM="$(find "$RAIZ/../thz-lsp-jvm/build/libs" "$RAIZ/JVM/thz-lsp-jvm/build/libs" "$RAIZ/target" -maxdepth 1 -name "thz-lsp*.jar" ! -name "*-sources.jar" 2>/dev/null | head -n 1)"
