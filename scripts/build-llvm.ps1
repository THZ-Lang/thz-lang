...
if ($Alvo -eq "ambos" -or $Alvo -eq "windows") {
    ...
    # Linker com Runtime Nativo Rust (../thz-runtime-rs ou src/runtime_rs)
    $RuntimeRs = if (Test-Path "$Raiz\..\thz-runtime-rs\Cargo.toml") { "$Raiz\..\thz-runtime-rs" } else { "$Raiz\src\runtime_rs" }
