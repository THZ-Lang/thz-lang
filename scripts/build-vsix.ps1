...
$PastaExtensao = if (Test-Path (Join-Path $Raiz "..\thz-vscode")) { (Resolve-Path (Join-Path $Raiz "..\thz-vscode")).Path } else { Join-Path $Raiz "Extensions\thz-lsp-vscode" }
...
$LspJarItem = Get-ChildItem (Join-Path $Raiz "..\thz-lsp-jvm\build\libs\thz-lsp*.jar"), (Join-Path $Raiz "JVM\thz-lsp-jvm\build\libs\thz-lsp*.jar"), (Join-Path $Raiz "target\thz-lsp*.jar") -ErrorAction SilentlyContinue | Where-Object { $_.Name -notmatch "-sources" } | Select-Object -First 1
