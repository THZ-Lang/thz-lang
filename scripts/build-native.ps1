...
if (-not $ApenasGui) {
    ...
    $JarCli = (Get-ChildItem "$Raiz\target\thz-jvm*.jar", "$Raiz\..\thz-cli-jvm\build\libs\thz-jvm*.jar", "$Raiz\JVM\thz-cli-jvm\build\libs\thz-jvm*.jar" | Where-Object { $_.Name -notmatch "-sources" } | Select-Object -First 1).FullName
...
if (-not $ApenasCli) {
    ...
    $JarGui = Get-ChildItem "$Raiz\..\thz-gui-jvm\build\libs\thz-gui-jvm-*-all.jar", "$Raiz\..\thz-gui-jvm\build\libs\thz-gui-jvm-*.jar", "$Raiz\JVM\thz-gui-jvm\build\libs\thz-gui-jvm-*-all.jar", "$Raiz\JVM\thz-gui-jvm\build\libs\thz-gui-jvm-*.jar" -ErrorAction SilentlyContinue | Select-Object -First 1
    if (-not $JarGui) {
        # Fallback de busca
        $JarGui = Get-ChildItem "$Raiz\..\thz-gui-jvm\build\libs\*.jar", "$Raiz\JVM\thz-gui-jvm\build\libs\*.jar" | Select-Object -First 1
    }
