...
- [x] **Fase 6 (G3):** LSP + Extensão VS Code — src/lsp/server.ts + extension/ (hover, diagnósticos, símbolos, completion, go-to-definition).
...
- LSP + VS Code: `src/lsp/server.ts` (stdio, diagnostics/hover/symbols/completion/definition/formatting) + `extension/` (TextMate, LanguageClient) — `npm run lsp` / `npm run extension:compile`.
